...
def explode_nominee_json(nominees_df: pd.DataFrame, json_col: str = "nominee") -> pd.DataFrame:
    """
...
def enrich_congress_nominees_dataframe(
    ...
    Returns:
        DataFrame with additional name field
    """
    # Create a copy to avoid modifying the original
...
def extract_last_name(full_name):
    """
    Extract just the last name from a full name string.
    Handles names in format "FirstName LastName" or "FirstName MiddleName LastName"
    Excludes suffixes like Jr., Sr., III, etc.
    
    Args:
        full_name: String containing the full name
        
    Returns:
        String containing just the last name
    """
    if not isinstance(full_name, str) or not full_name.strip():
        return ""
    
    # Remove common suffixes before processing
    suffixes = [r'\bJr\.?\b', r'\bSr\.?\b', r'\bI{1,3}V?\b', r'\bIV\b']
    clean_name = full_name
    
    for suffix_pattern in suffixes:
        clean_name = re.sub(suffix_pattern, '', clean_name).strip()
